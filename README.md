...
- 일반용 재구성은 원문 근거를 한국어로 정확히 전달하는 것을 우선하고, 초등 고학년용 재구성은 같은 근거를 더 쉬운 문장으로 풀되 핵심 용어·수치·조사대상·시점은 빼지 않습니다.
- `본문을 옮기지 않고`, `원문 링크로만`, `최소 설명만 제공`해야 하는 자료에는 카드 안 재구성 본문을 두지 않습니다.
- 모든 외부 자료 카드는 `재구성 본문 제공`, `원문 링크만 제공`, `감사된 표·수치만 재생산` 중 하나로 분명히 분류합니다.
