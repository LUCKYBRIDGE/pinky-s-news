...
- 각 질문은 `questionTypes`와 `elementaryQuestionTypes`에서 `토론 질문` 또는 `토의 질문`으로 구분하고, 화면에서는 색 배지로 표시합니다.
- 화면에서는 주제 전체의 `찬반 토론형`/`해결 설계형 토의` 배지를 표시하지 않고, 선택 가능한 질문 단위의 유형만 보여줍니다.
